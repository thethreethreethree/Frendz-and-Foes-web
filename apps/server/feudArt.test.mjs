...
// asset went undetected. Slugs are always string literals, so require the surrounding quote.
const referenced = (slug) =>
  code.includes('"' + slug + '"') || code.includes("'" + slug + "'") ||
  code.includes(String.fromCharCode(96) + slug + String.fromCharCode(96));
const unreferenced = applied.filter((m) => !referenced(m.slug));
...
const wiredReject = rejected.filter((m) => m.slug && referenced(m.slug));
