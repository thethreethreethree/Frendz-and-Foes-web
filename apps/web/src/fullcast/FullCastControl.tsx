...
// phrase; the whole team mimes it while one guesser (facing away / watching nothing) shouts guesses.
// An actor taps got/skip. The guesser must not see this screen or the TV (which stays phrase-free).

const TEAM_COLORS = ["#e11d48", "#2563eb", "#059669", "#d97706", "#7c3aed", "#0ea5e9"];
const uid = () => Math.random().toString(36).slice(2, 8);

export function FullCastControl() {
  const g = useFullCastHost();
  const { state } = g;
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 text-center">
      ...
      <div>
        ...
        <p className="mt-3 max-w-xs text-sm text-muted">Pick ONE guesser to look away. Everyone else gathers around this phone — you all act out each phrase together. Tap ✓ when they guess it.</p>
      </div>
  ...
  return (
    <div className="flex flex-1 flex-col gap-3">
      ...
      <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-line bg-surface p-5 text-center">
        <div className="text-xs font-semibold uppercase tracking-wide text-muted">All of you act out</div>
        <div className="ff-title mt-2 text-4xl">{card?.word ?? "…"}</div>
      </div>
      <div className="grid grid-cols-[1fr_auto] gap-2">
        <button onClick={g.got} className="ff-sticker bg-success px-4 py-5 font-display text-2xl text-canvas">✓ GUESSED</button>
