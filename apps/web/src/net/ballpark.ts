...
export interface BpPlayer { id: string; name: string; connected: boolean; score: number; guessed: boolean; bet: boolean }
...
export interface BpYou { id: string; name: string; rejoinToken?: string }

export const bpSync = (room: string) => getSocket().emit("bp:sync", { room });
export const bpJoin = (room: string, name: string, playerId?: string, rejoinToken?: string) =>
  getSocket().emit("bp:join", { room, name, playerId, rejoinToken });
...
export function loadBpPlayer(room: string): { id?: string; name?: string; rejoinToken?: string } {
  try { return JSON.parse(localStorage.getItem(key(room)) || "{}"); } catch { return {}; }
}
export function saveBpPlayer(room: string, v: { id?: string; name?: string; rejoinToken?: string }) {
