...
export interface TePlayer { id: string; name: string; connected: boolean; submitted?: boolean }
...
export interface TeYou {
  id: string; name: string; rejoinToken?: string;
  phase: string;
  turn?: number;
  type?: "draw" | "text";
...
export const teJoin = (room: string, name: string, playerId?: string, rejoinToken?: string) =>
  getSocket().emit("te:join", { room, name, playerId, rejoinToken });
...
export function loadTePlayer(room: string): { id?: string; name?: string; rejoinToken?: string } {
  try { return JSON.parse(localStorage.getItem(key(room)) || "{}"); } catch { return {}; }
}
export function saveTePlayer(room: string, v: { id?: string; name?: string; rejoinToken?: string }) {
